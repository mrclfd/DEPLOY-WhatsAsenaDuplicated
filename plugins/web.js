...
Asena.addCommand({pattern: 'ping', fromMe: true, deleteCommand: false, desc: Lang.PING_DESC}, (async (message, match) => {
  ...
  await message.client.sendMessage( message.jid, '```Ping!```', MessageType.text, {contextInfo: { forwardingScore: 666, isForwarded: true }, sendEphemeral: 'chat'});
  var end = new Date().getTime();

  await message.client.sendMessage(
    message.jid,'*Pong!*\n```' + (end - start) + 'ms```', MessageType.text, {contextInfo: { forwardingScore: 666, isForwarded: true }, sendEphemeral: 'chat'});
}));
